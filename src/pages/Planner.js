...
const Planner = () => {
    ...
    const handleCheck = (id) => {
        setToDoList((prevList) => 
            prevList.map((item) => 
                item.id === id ? { ...item, completed: !item.completed, completedDate: formatDate(new Date()) } : item
            )
        );
            ...
            setToDoList((prevList) => {
                ...
                if (itemToMove && itemToMove.completed) {
                    setCompletedList((prevCompleted) => {
                        if (!prevCompleted.some((completedItem) => completedItem.id === id)) {
                            return [...prevCompleted, itemToMove].sort((a, b) => new Date(b.completedDate) - new Date(a.completedDate));
                        }
    ...
    const handleAddItem = () => {
        const newItem = { id: Date.now(), text: '할 일을 입력하세요', completed: false };
        setToDoList((prevList) => [...prevList, newItem]);
    };

    const sortedCompletedList = completedList.sort((a, b) => 
